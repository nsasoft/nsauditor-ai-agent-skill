...
SKILL.md now teaches the `scan_cloud` `regions` argument: AWS region codes (e.g. `["us-east-1","eu-west-1"]`) or `["all"]` to fan out across every account-enabled region, with the divergent default (omit = the server-configured `AWS_REGION`; omitting does NOT fan out — pass `["all"]` explicitly) and UX guidance to offer `["all"]` when the user asks for full / every-region coverage — reserved for explicit full-coverage requests, since it is slower and can approach the Claude Desktop MCP tool-call timeout on large accounts. Paired with the EE 0.17.0 `--aws-region` feature + CE 0.2.0. Plugin count UNCHANGED (28); all six matrices UNCHANGED.
