...
> **Version:** 0.2.28 (post-EE-0.32.5 — a **matrix-neutral** report-quality and routing-integrity release: nine API Gateway mapping rules that matched zero findings repaired (routing was never broken; the defect was a **stale** auditor-facing rationale rendering in place of the accurate one), the HIPAA §164.312(c)(1) WAF mapping removed on doctrine rather than repaired, six rationales rewritten out of internal development narration, `renderJSON` no longer shipping raw routing regexes, a CI/CD AccessDenied false-negative closed fail-closed, and three mutation-proven compliance-guard hardenings. Plugin count UNCHANGED at 28; all seven coverage matrices UNCHANGED.)
