...
| `scan_cloud` | Enterprise | Audit one or more cloud accounts (AWS / GCP / Azure) for security & compliance posture using the server-configured credentials. No network host needed. Input: `{ providers?: ("aws"\|"gcp"\|"azure")[], regions?: string[] }` — **pass only the cloud(s) the user names** (`providers:["aws"]` for "audit my AWS account"); omit `providers` only when the user asks to audit ALL clouds. Use this (not `scan_host`) when the user asks to "audit my AWS account", "audit my AWS and Azure accounts", or "check my cloud compliance". CE/Pro callers get an upgrade message. **`regions` (AWS only)** — AWS region codes (e.g. `["us-east-1","eu-west-1"]`) or `["all"]` to fan out across every account-enabled region. **Divergent default: OMIT `regions` to audit the server-configured `AWS_REGION` (single/env default); omitting does NOT scan all regions.** When the user asks for "all regions", "every region", "my whole AWS account everywhere", or "complete/full coverage", pass `regions:["all"]`; when they name specific region(s), pass those. Caveat — `["all"]` fans regional plugins (security groups, RDS, KMS, Lambda, etc.) across all enabled regions and is slower; it can approach the Desktop MCP tool timeout on large accounts, so reserve it for when the user explicitly wants full-region coverage. Unknown region codes are rejected before the scan runs. |
