...
> **Version:** 0.2.27 (post-EE-0.32.4 — a **matrix-neutral** RDS false-negative-and-report-quality depth pass on plugin 1140: **no new framework, no new plugins (still 28), all seven coverage matrices UNCHANGED.** The RDS auditor gains **RDS Proxy client↔proxy TLS** (`DescribeDBProxies.RequireTLS` — a proxy that does not require TLS accepts cleartext client connections, a transit leg distinct from the DB-engine SSL parameter; fail-closed on false-or-absent → routes == the DB-SSL transit axis), a new **retained / cross-region-replicated automated-backup at-rest surface** (`DescribeDB{Instance,Cluster}AutomatedBackups` — an unencrypted automated backup that survives instance/cluster deletion is invisible to the live-resource and snapshot scans), and the **Aurora cluster-member double-audit closure** (a provisioned Aurora cluster's members defer the cluster-scoped SSL / Multi-AZ dims to the cluster, closing self-contradictory instance-level false positives + a headless-Aurora availability false-clean, keyed on `EngineMode`). A renderer backstop also closes a **cross-framework report-quality leak** (foreign framework control-ids stripped out of the violation prose that renders into every framework report — all channels). Prior context — EE 0.32.3 added Aurora **cluster-level SSL enforcement** (`DescribeDBClusterParameters`, closing an instance-less Aurora Serverless v1 cleartext false-negative) + **staged-parameter (`ParameterApplyStatus`) discipline** + the CE **GRC-push startup preflight** (CLI-only, not MCP-reachable); EE 0.32.2 completed the **Vanta · Drata · Secureframe** GRC connector trio (records model, opt-in early-access, outbound single-workspace, live-tenant validation deferred to partner intake) + closed the cross-framework foreign-token leak in the "Why this violates" rationales; EE 0.32.1 cleaned internal engineering markers out of the rationales. Honest status: the three connectors are shipped + opt-in + extensively tested; live validation against production tenants in progress — early-access, single-workspace, not a multi-tenant sync. The seven frameworks: SOC 2 · HIPAA · NIST CSF 2.0 · PCI DSS v4.0.1 · ISO/IEC 27001:2022 · CIS Controls v8 · **GDPR Article 32** — the last is **Art. 32 infrastructure substrate ONLY, NOT GDPR compliance** [Art. 32 is the only one of GDPR's 99 articles an infrastructure scanner can evidence; the rest is operator-side, out of scope by design], carrying four-factor proportionality + personal-data-scope attestation + the Art. 83(4) LOWER fine tier [€10M/2%, not the €20M/4% headline tier]. Paired CE 0.2.29 + EE 0.32.4) · **Source:** [github.com/nsasoft/nsauditor-ai](https://github.com/nsasoft/nsauditor-ai) · **npm:** `nsauditor-ai` · **License:** MIT (CE)
