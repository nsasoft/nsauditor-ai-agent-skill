...
> **Version:** 0.2.46 (knowledge current as of **EE 0.41.0** · **requires CE ≥ 0.2.45**) — the DocumentDB release.
>
> **What this release (EE 0.41.0) teaches:** **The 29th Enterprise plugin — `1230 AWS DocumentDB Auditor` — owns the `docdb` engine on the shared RDS control plane.** Teach these three facts together: (1) a `--host aws` pass now includes DocumentDB cluster security — storage encryption + KMS custody, the `tls` cluster parameter, `audit_logs` + CloudWatch export, backup retention, deletion protection, replica/AZ topology (DocumentDB availability is replicas across AZs; the per-instance Multi-AZ flag does not apply and the plugin never consults it), and manual-snapshot public/cross-account restorability; (2) the RDS auditor (1140) engine-filters DocumentDB and Neptune out of every path with counted skips and a standing deferred-scope disclosure — when a user asks about Neptune, the honest answer is that **a Neptune estate is UNAUDITED by every plugin**, and the report says so itself; (3) DocumentDB ELASTIC clusters ride a separate API and are a declared deferred scope, not covered. Plugin count 28 → 29; all eight coverage matrices UNCHANGED.

> **Prior:** 0.2.45 (post-EE-0.40.3 · **requires CE ≥ 0.2.45**) — the message-imprint release.
>
