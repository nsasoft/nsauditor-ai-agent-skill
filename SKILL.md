...
> **Version:** 0.2.26 (post-EE-0.32.3 — a **matrix-neutral** RDS false-negative depth pass: **no new framework, no new plugins (still 28), all seven coverage matrices UNCHANGED.** Plugin 1140 now audits Aurora **cluster-level SSL enforcement** (`DescribeDBClusterParameters`), closing a cleartext false-negative on **instance-less Aurora Serverless v1** clusters the instance-level SSL check never saw (a cluster whose cluster parameter group set `rds.force_ssl=0` / `require_secure_transport=OFF` produced zero transit-encryption findings → now CRITICAL), and applies **staged-parameter (`ParameterApplyStatus`) discipline** so a set-but-not-yet-applied `rds.force_ssl` / `pgaudit` is no longer affirmed as effective. The CE CLI adds a **GRC-push startup preflight** — fail-fast on a bad GRC token / control-map / provider at scan startup before the full scan (CLI-only, not MCP-reachable). The new cluster findings route to the existing transit-encryption controls (titlePattern-only anchor edit). Prior context — EE 0.32.2 completed the **Vanta · Drata · Secureframe** GRC connector trio (records model, opt-in early-access, outbound single-workspace, live-tenant validation deferred to partner intake) + closed the cross-framework foreign-token leak in the "Why this violates" rationales; EE 0.32.1 cleaned internal engineering markers out of the rationales. Honest status: the three connectors are shipped + opt-in + extensively tested; live validation against production tenants in progress — early-access, single-workspace, not a multi-tenant sync. The seven frameworks: SOC 2 · HIPAA · NIST CSF 2.0 · PCI DSS v4.0.1 · ISO/IEC 27001:2022 · CIS Controls v8 · **GDPR Article 32** — the last is **Art. 32 infrastructure substrate ONLY, NOT GDPR compliance** [Art. 32 is the only one of GDPR's 99 articles an infrastructure scanner can evidence; the rest is operator-side, out of scope by design], carrying four-factor proportionality + personal-data-scope attestation + the Art. 83(4) LOWER fine tier [€10M/2%, not the €20M/4% headline tier]. Paired CE 0.2.28 + EE 0.32.3) · **Source:** [github.com/nsasoft/nsauditor-ai](https://github.com/nsasoft/nsauditor-ai) · **npm:** `nsauditor-ai` · **License:** MIT (CE)
