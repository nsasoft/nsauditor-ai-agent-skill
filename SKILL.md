...
> **Version:** 0.2.24 (post-EE-0.32.1 — a **matrix-neutral compliance report-quality hygiene + deeper positive-substrate curation + GRC-connector DRY refactor** patch: **no new framework, no new plugins (still 28), all seven coverage matrices UNCHANGED.** EE 0.32.1 cleans internal engineering markers (`[[wiki-links]]`, `EE-RT` work-codes, reviewer-codes, `Rn-SEVERITY` review-round IDs) out of the "Why this violates" rationales that render into every Report on Compliance across all seven frameworks (~900 rationales, subsequence-invariant-proven), fixes a KMS-parse-failure rationale that leaked a reviewer-code + foreign token into all seven reports, opts more Azure + deeper-AWS PASS-tier findings into the display-only positive-substrate RoC view, and collapses the Vanta + Drata push loops into a shared `_runPushBatch`. Prior context — EE 0.32.0 wires the dormant Vanta connector for **scan-time GRC push** — set `COMPLIANCE_GRC_PROVIDER=vanta` + `COMPLIANCE_GRC_TOKEN` and the compliance phase maps each finding to Vanta test results and pushes them (opt-in, ZDE-redacted egress, token never serialized; suppression-aware outcomes, framework-dimensioned idempotency, rate-limit backoff, circuit breaker) — and ships a new **Drata** connector (Custom Connections; your Drata Test Builder rules evaluate the pushed records); Secureframe on the roadmap. Honest status: connectors shipped + opt-in + extensively tested; live validation against production tenants in progress — early-access, single-workspace, not a multi-tenant sync. Also opts 60 AWS PASS-tier findings (T1 at-rest + T2 AWS-other) into the per-control RoC positive-substrate view (display-only + non-flipping + count-neutral). The seven frameworks: SOC 2 · HIPAA · NIST CSF 2.0 · PCI DSS v4.0.1 · ISO/IEC 27001:2022 · CIS Controls v8 · **GDPR Article 32** — the last is **Art. 32 infrastructure substrate ONLY, NOT GDPR compliance** [Art. 32 is the only one of GDPR's 99 articles an infrastructure scanner can evidence; the rest is operator-side, out of scope by design], carrying four-factor proportionality + personal-data-scope attestation + the Art. 83(4) LOWER fine tier [€10M/2%, not the €20M/4% headline tier]. Paired CE 0.2.26 + EE 0.32.1) · **Source:** [github.com/nsasoft/nsauditor-ai](https://github.com/nsasoft/nsauditor-ai) · **npm:** `nsauditor-ai` · **License:** MIT (CE)
...
| `COMPLIANCE_GRC_PROVIDER` | — | **Enterprise** — opt-in scan-time GRC push: `vanta` or `drata` (Secureframe planned). Needs `COMPLIANCE_GRC_TOKEN`; optional `COMPLIANCE_GRC_BASE_URL` / `COMPLIANCE_GRC_CONTROL_MAP` / `COMPLIANCE_GRC_REDACTION` (`off`/`hash`/`remove`). Egress is ZDE-redacted; token never serialized. Live tenant validation in progress. |
