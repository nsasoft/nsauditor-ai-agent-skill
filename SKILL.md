...
> **Version:** 0.2.31 (post-EE-0.32.8 — a **documentation-and-prose** release, no behaviour change. Capability-claim honesty pass, part 2: the air-gapped-delivery class. **27 withdrawn-claim sites** (distinct lines) across the three published packages, each verified against the code — an arm64 image (the build forces `--platform linux/amd64`), offline-installable tarballs (an `npm pack` tarball still resolves deps from a registry), monthly NVD feed bundles (nothing produces one), an air-gapped install script (none exists), a `feed import` CLI command (`importFeed()` has zero non-test callers), and CE's absolute "every feature works without internet access". **Five of the sites were rendered runtime strings**, not documentation: coverage-gap rationales in `utils/intelligence_engine.mjs` that render verbatim into the assessor-facing evidence pack and named a command that does not exist. **In this skill:** the Enterprise tier row's unhedged "RFC 3161 timestamps" is corrected — RFC 3161 timestamping is implemented but not yet wired to a flag, so **do not describe it as shipping**; SHA-256 chain-of-custody is the layer that ships. "Air-gapped deployment" is now stated as "air-gapped operation (offline licensing + offline CVE matching under `NSAUDITOR_OFFLINE_ONLY=1`)". Plugin count UNCHANGED at 28; all seven coverage matrices UNCHANGED. No MCP tool or plugin schema change.)
