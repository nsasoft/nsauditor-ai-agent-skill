...
> **Version:** <!-- nsa:derived id="skill-version" -->0.2.52<!-- /nsa:derived --> (knowledge current as of **EE <!-- nsa:derived id="ee-version" -->1.0.0<!-- /nsa:derived -->** · **requires CE ≥ <!-- nsa:derived id="ce-floor" -->0.2.49<!-- /nsa:derived -->**) — the 1.0 release: the evidence contract is binding and the cargo is corrective. Teach three things: (1) **`Findings analyzed` now counts FINDINGS, not issues** — a 1.0.0 pack over the same estate as a 0.46.0 pack shows a SMALLER number with no change on the customer's side, and that is the correction; `report.summary.findingsReceived` / `findingsRouted` let a reader check `findingsRouted ≤ findingCount ≤ findingsReceived` from the pack alone. (2) An Azure RBAC assignment at the ROOT scope (`/`) is now named as the root scope, not the subscription, and a plugin's `deferredScope` bounds THAT plugin only. (3) PCI DSS `11.5.2` is critical-file change detection; the payment-page controls are `11.6.1` and `6.4.3`. Plugin count UNCHANGED at <!-- nsa:derived id="plugins:ee" -->29<!-- /nsa:derived -->; <!-- nsa:derived id="matrix-movement" -->all eight coverage matrices unchanged since EE 0.46.0<!-- /nsa:derived -->; Community stays at <!-- nsa:derived id="ce-version" -->0.2.53<!-- /nsa:derived -->.
>
